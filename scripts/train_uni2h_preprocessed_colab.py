"""Colab-ready UNI2-h (MahmoodLab) training with robust checkpointing to Google Drive.

Frozen ViT-H/14-reg8 encoder (hf-hub:MahmoodLab/UNI2-h) + linear head on preprocessed H5 patches.
Same training loop / metrics / checkpoint layout as train_virchow_preprocessed_colab.py.
"""
...
BATCH_SIZE = 48
EMBED_DIM = 1536
UNI2H_HF_ID = "hf-hub:MahmoodLab/UNI2-h"
...
def get_embedding(backbone, x):
    """x: (B, 3, 224, 224). Returns (B, 1536) pooled UNI2-h features."""
    with torch.no_grad():
        out = backbone(x)
    if out.dim() == 3:
        return out[:, 0]
...
def load_uni2h_backbone(device: torch.device):
    """Load MahmoodLab UNI2-h via timm (official kwargs from github.com/mahmoodlab/UNI)."""
    ...
    backbone = timm.create_model(UNI2H_HF_ID, pretrained=True, **uni2h_kwargs)
    return backbone.to(device).eval()


class Uni2hClassifier(nn.Module):
    """Frozen UNI2-h + optional dropout (for MC Dropout) + Linear(1536, 1)."""
...
def export_final_artifacts(
    ...
    run_manifest = {
        "created_by": "train_uni2h_preprocessed_colab.py export_final_artifacts",
...
def main():
    parser = argparse.ArgumentParser(description="Colab-ready UNI2-h (frozen) training on preprocessed PCam.")
    ...
    print("Loading UNI2-h backbone ({}) ...".format(UNI2H_HF_ID))
    try:
        backbone = load_uni2h_backbone(device)
        model = Uni2hClassifier(backbone, head_dropout_p=args.head_dropout).to(device)
    except Exception as e:
        print("Error: UNI2-h load failed:", e)
        print("Tip: accept the license at https://huggingface.co/MahmoodLab/UNI2-h and run `huggingface-cli login`.")
        sys.exit(1)
    ...
    run_config = {
        "script": "train_uni2h_preprocessed_colab.py",
