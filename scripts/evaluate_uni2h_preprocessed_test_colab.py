"""Evaluate trained UNI2-h head on reserved preprocessed PCam test split.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate reserved PCam test split with trained UNI2-h head.")
    ...
    print("[evaluate_uni2h] CLI OK; importing PyTorch / timm / helpers ...", flush=True)
    print(
        "[evaluate_uni2h] First-time CUDA init + large imports often take 1–5+ min on Colab; not stuck yet.",
        flush=True,
    )
    ...
    print("[evaluate_uni2h] PyTorch import + CUDA init finished.", flush=True)
    ...
    print("[evaluate_uni2h] Loading dataset/metrics helpers from train_uni2h_preprocessed_colab ...", flush=True)
    from train_uni2h_preprocessed_colab import (  # type: ignore
    ...
    print("[evaluate_uni2h] Imports finished.", flush=True)

    def _load_model_weights(model: Uni2hClassifier, run_dir: Path, device: torch.device) -> Path:
    ...
    print("[evaluate_uni2h] Opening HDF5 test split ...", flush=True)
    ...
    print("Loading UNI2-h backbone ({}) ...".format(UNI2H_HF_ID), flush=True)
    try:
        backbone = load_uni2h_backbone(device)
        model = Uni2hClassifier(backbone, head_dropout_p=head_dropout).to(device)
...
if __name__ == "__main__":
    print("[evaluate_uni2h] script entry (__main__)", flush=True)
    main()
