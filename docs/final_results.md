...
Table 10 gives held-out test metrics on quality-filtered, Macenko-normalized tensors scaled to \([0,1]\) (\(n_{\mathrm{test}} = 27{,}704\) for PCam test, 83,181 for CAMELYON17 test). Figure 9 shows confusion matrices at threshold 0.5 (row-normalized fractions with raw counts annotated). Table 12 reports transfer contrasts as \(\Delta = M_{\mathrm{in}} - M_{\mathrm{ext}}\) on calibrated probabilities.
...
Virchow2 was used with a frozen pretrained backbone and a trainable linear head (dropout 0.2). Four conditions (C1–C4) mirror the transfer layout: train on PCam or CAMELYON17, evaluate on the in-domain test split and on the external test split with the same study preprocessing pipeline as §7.2. Temperature was fit once per training run and applied to all test logits from that run.
...
The experiments trace a single pipeline from public patch benchmarks through shared quality control and Macenko stain normalization to two classifier families evaluated under strict transfer rules. The data stages clarify what was removed and why, while the modeling stages separate representation capacity from preprocessing and calibration choices.

This work should not be read as a universal remedy for domain shift. The contribution is methodological and empirical: a leakage-aware pipeline, bidirectional transfer between two patch benchmarks, and reporting that couples ranking with threshold and calibration behaviour. Cross-domain metrics improved in prespecified comparisons—most visibly when the shallow CNN moved from raw catalogue inputs to the full study pipeline on CAMELYON17, and when Virchow2 was compared to that CNN on identical preprocessed tensors—but the hardest direction (CAMELYON17-trained weights on external PCam) still showed a large gap between ROC–AUC and recall at 0.5, with worse probability reliability. Any claim about “generalization” is therefore limited to the behaviours measured on these fixed splits; it does not extend to slide-level deployment or to unseen hospitals without further validation.
...
The shallow CNN results establish how much performance depends on preprocessing and transfer direction. Without the study pipeline—quality control, stain normalization, and consistent value scaling—CAMELYON17-trained weights showed weak in-domain performance on raw CAMELYON17 test (ROC-AUC 0.72 with recall 0.16 at 0.5), which confirms that raw public patches alone were not a fair match to the curated corpora used elsewhere. After the full pipeline, the CNN reached strong in-domain and cross-domain ranking on CAMELYON17 when trained on PCam, mirroring a pattern also seen for Virchow2. The harder direction was always CAMELYON17-trained weights evaluated on PCam test: accuracy and recall fell sharply, false negatives dominated the error budget, and calibration error rose. The CNN suffered a larger absolute ROC-AUC drop in that direction than Virchow2, although both models retained ROC-AUC near or above 0.85 on external PCam.

Virchow2 improved every preprocessed comparison in Table 18 by a meaningful margin. The gain was not limited to in-domain PCam test; it persisted on external surfaces and was largest when CAMELYON17-trained weights were applied to PCam. In that cell, Virchow2 preserved substantially higher recall at similar precision, which explains why accuracy and proper scores moved together while ROC-AUC alone understated the operational gap. Bootstrap intervals were tight on large test sets but still excluded trivial shifts for paired contrasts on shared patches. Permutation tests on patch-wise Brier and log-loss confirmed that the two training origins produced different probability behaviour on the same tiles, not only different aggregate thresholds.

The asymmetric transfer pattern is the central empirical theme. When training on PCam, both models ranked tumour-positive patches above negatives on CAMELYON17 test at least as well as on PCam test. When training on CAMELYON17, both models still ranked reasonably well on external PCam by ROC-AUC, but threshold accuracy and recall collapsed for Virchow2 and fell further for the CNN. On C4, ROC-AUC remained high (0.950) while recall at 0.5 fell to 0.630 with precision near 0.99, indicating a miscalibrated operating point and a linear-head decision boundary tuned on CAMELYON17 validation rather than a complete loss of ranking signal from the frozen encoder. Calibration worsened in the same direction. Reliability diagrams and ECE values indicate that the model remained confident while missing many positives, a failure mode that is particularly risky if a fixed 0.5 cutoff is interpreted as a clinical decision rule without domain adaptation.
...
Taken together, the results portray domain shift as a graded phenomenon rather than a single drop in area under the curve. Preprocessing and a strong frozen encoder mitigated but did not remove transfer loss on the difficult direction. Ranking metrics can remain high while calibration and recall at a default threshold fail. A deployment that relies on probability magnitudes or on sensitivity at 0.5 would need domain-specific validation, threshold tuning, or further adaptation, even when offline ROC-AUC appears strong. Future work might target recall on external PCam through class-weighting, focal losses, or post-hoc threshold selection on validation material from the target domain, and might extend the qualitative protocol with formal inter-rater agreement on a larger stratified sample.

The resize-kernel and training-curve figures document implementation choices that are easy to overlook in tabular summaries alone. They show that reported metrics came from converged training runs and from a resize step that preserved sharper patch appearance in qualitative review. Further structural limits, compute bounds, and planned extensions are summarized in the Limitations and Future Work chapter. Within the scope of this evidence, the story is coherent: shared preprocessing enables cross-dataset benchmarking, foundation-model features improve discrimination and calibration over a shallow baseline on the same tensors, and the remaining stress concentrates on external PCam test when CAMELYON17-trained optimization has not matched PCam appearance at the chosen operating point.
